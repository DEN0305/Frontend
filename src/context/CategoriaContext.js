...
const CategoriaProvider = ({children}) => {
    ...
    let api = helpHttp();
    let url = REACT_APP_API_URL+"categoria";
    ...
    const saveData = (data) => {
        ...
        api.post(endpoint, options).then((res) => {
            if(!res.err){
                ...
                setType("success");
                setMessage("La categoría fue creada correctamente");
                setStatus(1);
            }else{
    ...
    const updateData = (data) => {
        ...
        api.put(endpoint, options).then((res) => {
            if(!res.err){
                ...
                navigate('/admin/categoria');
                setType("success");
                setMessage("La categoría fue actualizada correctamente");
                setStatus(1);
            }else{
    ...
    const deleteData = (id) => {
        ...
        api.del(endpoint, options).then((res) => {
            if(!res.err){
                ...
                setType("success");
                setMessage("La categoría fue eliminada correctamente");
                setStatus(1);
            }else{
                setType("danger");
    ...
    return <CategoriaContext.Provider value={data}>{children}</CategoriaContext.Provider>;
}
