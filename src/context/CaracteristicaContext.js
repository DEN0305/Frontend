    ...
    const saveData = (data) => {
        ...
        api.post(endpoint, options).then((res) => {
            if(!res.err){
                ...
                setType("success");
                setMessage("La característica fue creada correctamente");
                setStatus(1);
            }else{
    ...
    const updateData = (data) => {
        ...
        api.put(endpoint, options).then((res) => {
            if(!res.err){
                ...
                navigate('/admin/caracteristica');
                setType("success");
                setMessage("La característica fue actualizada correctamente");
                setStatus(1);
            }else{
    ...
    const deleteData = (id) => {
        ...
        api.del(endpoint, options).then((res) => {
            if(!res.err){
                ...
                setType("success");
                setMessage("La característica fue eliminada correctamente");
                setStatus(1);
            }else{
                setType("danger");
