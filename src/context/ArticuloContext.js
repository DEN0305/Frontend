    ...
    const saveData = (data) => {
        ...
        api.post(endpoint, options).then((res) => {
            if(!res.err){
                ...
                setType("success");
                setMessage("El artículo fue creado correctamente");
                setStatus(1);
            }else{
                setType("danger");
                setMessage(res.message || "Error al crear el artículo");
                setStatus(1);
            }
            setLoading(false);
        }).catch((error) => {
            setType("danger");
            setMessage("Error de conexión al crear el artículo");
            setStatus(1);
            setLoading(false);
        })
    }
    ...
    const updateData = (data, id) => {
        ...
        api.put(endpoint, options).then((res) => {
            if(!res.err){
                ...
                navigate('/admin/articulo');
                setType("success");
                setMessage("El artículo fue actualizado correctamente");
                setStatus(1);
            }else{
                setType("danger");
                const errorMessage = res.message?.message || res.message || "Error al actualizar el artículo";
            ...
            setType("danger");
            setMessage("Error de conexión al actualizar el artículo");
            setStatus(1);
            setLoading(false);
        })
    }
    ...
    const deleteData = (id) => {
        ...
        api.del(endpoint, options).then((res) => {
            if(!res.err){
                ...
                setType("success");
                setMessage("El artículo fue eliminado correctamente");
                setStatus(1);
            }else{
                setType("danger");
